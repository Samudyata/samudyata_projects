...
BASE_PATH = '/home/sjagird1/DL Project/Emotions_all/Emotions/'  # Update this path
...
def extract_melspectrogram(data, sr=22050):
    """Extract mel-spectrogram features from audio data with robust NaN handling"""
    try:
        # Ensure data is not empty or zero
        if len(data) == 0 or np.all(data == 0):
            print("Warning: Empty or zero audio data")
            return None
        ...
        if std == 0:
            print("Warning: Zero standard deviation in spectrogram")
            return None
...
def main():
    ...
    for idx, (path, emotion) in enumerate(zip(data_path.Path, data_path.Emotions)):
        ...
        try:
            ...
            # Skip if mel_spec is None
            if mel_spec is None:
                print(f"Skipping problematic file: {path}")
                continue
    ...
    model.save('final_model_conv2d_1K_1.keras')
